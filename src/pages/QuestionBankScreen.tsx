...
export const QuestionBankScreen: React.FC<QuestionBankScreenProps> = ({
  ...
  useEffect(() => {
    setPage(1);
  }, [selectedSubject, selectedCategory, selectedChapter, questionTypeFilter, searchQuery]);
  ...
  const subjectChapters = useMemo(() => {
    ...
    const directChapters = CHAPTERS_DATA.filter((c) => c.subject_id === selectedSubject.id);
  ...
  const categoryCounts = useMemo(() => {
    ...
    CATEGORY_CARDS_CONFIG.forEach((cat) => {
      ...
      // If dataset has limited tagged sample items for specific category, fallback to proportional count or default sample count
  ...
  const currentQuestions = useMemo(() => {
    ...
    // Apply category filter (if questions match category, or fallback to all subject questions if category specific list is empty)
    ...
    if (selectedChapter && selectedChapter !== 'all') {
      const chapterMatched = list.filter((q) => q.chapter_id === selectedChapter.id || q.chapter_name.includes(selectedChapter.bangla_name));
    ...
    return list;
  }, [questions, selectedSubject, selectedCategory, selectedChapter, questionTypeFilter, searchQuery]);
  ...
  // ==========================================
  // LEVEL 3 & 4: Questions List with Sticky Horizontal Chapter Chips (Matching Screenshots 1-4)
  // ==========================================
  if (selectedSubject && selectedCategory) {
    ...
    return (
      <div className="space-y-4 pb-32 max-w-2xl mx-auto px-1 sm:px-2">
        ...
        {paginatedQuestions.length > 0 ? (
          ...
          <EmptyState
            ...
            description="নিচের চিপ থেকে অন্য অধ্যায় বা ক্যাটাগরি নির্বাচন করুন অথবা ফিল্টার রিসেট করুন।"
            actionLabel="সকল অধ্যায় দেখুন"
            onAction={() => setSelectedChapter('all')}
          />
        )}

        {/* Sticky Bottom Bar: Horizontal Scrollable Chapter Chips + MCQ/Written Toggle (Matching Screenshots 1-4) */}
        <div className="fixed bottom-0 left-0 right-0 z-30 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-t border-slate-200/90 dark:border-slate-800 py-2.5 px-3 max-w-2xl mx-auto shadow-lg space-y-2">
          {/* Row 1: Horizontal Scrollable Chapter Chips */}
          <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar text-xs">
            ...
            {subjectChapters.map((ch) => {
              ...
              return (
                ...
                >
                  {ch.bangla_name} ({count})
                </button>
              );
            })}
          </div>

          {/* Row 2: MCQ vs Written Filter Radio/Pills (Matching Screenshot 4) */}
