...
export const ProgressScreen: React.FC<ProgressScreenProps> = ({
  ...
  return (
    <div className="space-y-3.5 pb-20 max-w-3xl mx-auto px-3 sm:px-4">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-[#E2E8F0] dark:border-slate-700 shadow-xs space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-bold text-[#1E3A8A] dark:text-indigo-400 text-sm sm:text-base">দৈনিক অর্জিত পয়েন্ট</h3>
            <p className="text-[11px] text-slate-500 dark:text-slate-400">গত ৭ দিনের প্র্যাকটিস ও সক্রিয়তা</p>
          </div>
          <div className="flex items-center gap-1.5 px-2.5 py-1 bg-amber-50 dark:bg-amber-950/50 text-amber-800 dark:text-amber-300 border border-amber-200 dark:border-amber-800 rounded-md text-xs font-bold font-mono">
              ...
              <div key={idx} className="flex flex-col items-center gap-1.5">
                ...
                <div className="w-full h-24 bg-slate-100 dark:bg-slate-700/60 rounded-lg flex items-end p-0.5 overflow-hidden">
                  <div
                    className={`w-full rounded-md transition-all duration-500 ${
                      isToday
                        ? 'bg-[#1E3A8A] dark:bg-indigo-500'
                        : pts > 0
                        ? 'bg-[#2563EB] dark:bg-indigo-400'
                ...
                <span
                  className={`text-[11px] font-bold ${
                    isToday ? 'text-[#1E3A8A] dark:text-indigo-400 font-black underline' : 'text-slate-400 dark:text-slate-500'
                  }`}
                >
                  {day}
                </span>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        <div className="high-density-stat-box">
          ...
          <div className="text-xl font-bold font-mono text-[#1E3A8A] dark:text-indigo-400">
        ...
        <div className="high-density-stat-box">
          ...
          <div className="text-xl font-bold font-mono text-[#10B981] dark:text-emerald-400">
            {isLoggedIn ? `${accuracy}%` : '০%'}
          </div>
          <div className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">সঠিক উত্তরের হার</div>
        </div>

        <div className="high-density-stat-box">
          <div className="text-[11px] text-slate-500 dark:text-slate-400 font-semibold mb-0.5">সঠিক উত্তর</div>
          <div className="text-xl font-bold font-mono text-[#2563EB] dark:text-blue-400">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-[#E2E8F0] dark:border-slate-700 shadow-xs space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-[#1E3A8A] dark:text-indigo-400 text-sm sm:text-base">বিষয়ভিত্তিক প্রস্তুতি অগ্রগতি</h3>
        ...
        <div className="space-y-2.5">
          <div>
            <div className="flex justify-between text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">
              ...
              <span className="font-mono text-[#1E3A8A] dark:text-indigo-400">
                {isLoggedIn ? `${Math.min(100, Math.max(10, progress.totalCorrect * 5 + 15))}%` : '০%'}
              </span>
            </div>
            <div className="w-full h-1.5 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
              <div
                className="h-full bg-[#1E3A8A] dark:bg-indigo-500 rounded-full transition-all duration-500"
          ...
          <div>
            <div className="flex justify-between text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">
              ...
              <span className="font-mono text-[#10B981] dark:text-emerald-400">
                {isLoggedIn ? `${Math.min(100, Math.max(5, progress.totalCorrect * 2 + 5))}%` : '০%'}
              </span>
            </div>
            <div className="w-full h-1.5 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
              <div
                className="h-full bg-[#10B981] dark:bg-emerald-500 rounded-full transition-all duration-500"
      ...
      <div className="bg-[#1E3A8A] text-white rounded-xl p-4 shadow-sm border border-blue-900 flex items-start gap-3.5">
        <div className="w-9 h-9 rounded-lg bg-blue-700 flex items-center justify-center text-amber-300 shrink-0">
